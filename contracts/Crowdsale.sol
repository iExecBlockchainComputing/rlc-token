...
contract Crowdsale is SafeMath, PullPayment, Pausable {
  ...
  modifier minCapNotReached() {
    if ((now<endBlock) || isMinCapReached() || (now > safeAdd(endBlock, 15 days))) throw;
    _;
  }

  /*
   *  /!\ FUNCTION FOR TEST ONLY - WILL BE REMOVED IN THE FINAL CONTRACT
   */
  ...
  function Crowdsale(address _token, address _btcproxy) {
    ...
    endBlock =  safeAdd(now, 30 days);    // ever (testnet) startdate + 30 days
    ...
    RLCEmitted = safeAdd(rlc_bounty, safeAdd(rlc_reserve, rlc_team));
  }
  ...
  function isMinCapReached() internal constant returns (bool) {
    return (safeAdd(RLCSentToETH, RLCSentToBTC ) > minCap);
  }

  function isMaxCapReached() internal constant returns (bool) { 
    return safeAdd(RLCSentToETH, RLCSentToBTC ) == maxCap;
  }

  // Compute the variable part
  function emitRLC(uint amount) internal {
    Logs(msg.sender ,amount, "emitRLC");
    rlc_bounty = safeAdd(rlc_bounty, amount/10);                 // bounty is 10% of the crowdsale
    rlc_team = safeAdd(rlc_team, amount/20);                     // team is 5% of the crowdsale
    rlc_reserve = safeAdd(rlc_reserve, amount/10); 	           // contingency is 10% of the crowdsale
    RLCEmitted = safeAdd(RLCEmitted, safeAdd(amount, amount/4)); // adjust the total number of RLC emitted
  }
  ...
  */
  ...
  function finalize() onlyBy(owner) {
    if (!isMaxCapReached() && now < safeAdd(endBlock, 15 days)) throw;
    //moves the remaining ETH to the multisig address
    if (!multisigETH.send(this.balance)) throw;
    //moves RLC to the team, reserve and bounty address
    if (!transferRLC(team,rlc_team)) throw;
    if (!transferRLC(reserve,rlc_reserve)) throw;	
    if (!transferRLC(bounty,rlc_bounty)) throw;
    rlc.burn(safeSub(rlc.totalSupply(), RLCEmitted));
    rlc.unlock();
    crowdsaleClosed = true;
  }
}
