...
// first payment in BTC with account 4
        return CrowdContract.receiveBTC(thirdcust, "0x004", 2000000, "tsxid", {from:btcproxy ,gas:3000000});
      }).then(function(result){
        return RLCcontract.balanceOf.call(thirdcust);
      }).then(function(result){
        console.log("payment 1 acc4 via BTC",result.toNumber());
        assert.equal(result.toNumber(),120000000000,"RLC send")  
...
// second payment in BTC with account 4, reached the min cap
        ...
        assert.equal(result.toNumber(),240000000000,"RLC sent min cap reached")  
